...
import { getProfile, saveProfile, createApplication, getOrCreateProfile, removeCredential } from '../../lib/store';
...
export default function ProfilePage({ params }: { params: Promise<{ wallet: string }> }) {
  ...
  useEffect(() => {
    ...
    // Immediate sync check — this is the key fix for the Edit button
  ...
  const handleGenerateJobCode = () => {
    ...
    const code = createApplication(wallet, jobForm.company, jobForm.title);
    const updated = getProfile(wallet); // reload to get the new app code
